...
const VinSearch = () => {
  ...
  const storedVin = useSelector(
    (state) => state?.vehicleForm?.vehicleValues?.vin
  )
  const [vin, setVin] = useState()
  const [getVehicle, { loading }] = useLazyQuery(VIN_SEARCH, {
    variables: { vin },
  ...
  return (
    <div className="max-w-xs mx-auto w-full lg:max-w-md">
      ...
      <form
        className="grid grid-cols-1 gap-4 items-start lg:grid-cols-3 lg:gap-2"
        onSubmit={search}
      >
        ...
        <div className="grid grid-cols-1 gap-4">
          <button
            type="submit"
            className="justify-center inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500"
          >
            {loading ? <Spinner /> : 'Go'}
          </button>
        </div>
