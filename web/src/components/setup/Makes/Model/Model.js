...
const Model = ({ model }) => {
  ...
  const [updateModelCategoryInput, {loading, data, error}] = useMutation(UPDATE_MODEL_CATEGORY, {
    onCompleted: () => {
  ...
  useEffect(() => {
    if (categoryResults.data) {
      setCategories([defaultCategory, ...categoryResults.data.categoriesForCompany])

      if (model.category) {
        setSelected(categoryResults.data.categoriesForCompany.find(cat => cat.id === model.category.id))
      }
    }
  }, [categoryResults])
        ...
        <div className="md:grid md:grid-cols-3 md:gap-6">
          <div className="md:col-span-1">
            <div className="px-4 sm:px-0">
              <h3 className="text-lg font-medium leading-6 text-gray-900">{model.name}</h3>
              <p className="mt-1 text-sm text-gray-600">
                Assign the {makeName} {model.name} to a category, or choose to not support it.
              </p>
            </div>
          </div>
              ...
              <div className="shadow sm:rounded-md">
                <div className="px-4 py-5 bg-white space-y-6 sm:p-6">
                  <div className="grid grid-cols-3 gap-6">
                    <div className="col-span-3 sm:col-span-2">
                      ...
                      {({ open }) => (
                        <>
                          <Listbox.Label className="block text-sm font-medium text-gray-700">Current Category</Listbox.Label>
                ...
                <div className="px-4 py-3 bg-gray-50 text-right sm:px-6">
                  <button
                    ...
                    onClick={() => navigate(routes.contractGeneratorModels({makeName}))}
                  >
                    Back
                  </button>
                  <Submit
