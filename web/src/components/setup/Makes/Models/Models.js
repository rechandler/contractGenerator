...
const Models = ({ models }) => {
  ...
  useEffect(() => {
    if (categoryResults.data) setCategories([defaultCategory, ...categoryResults.data.categoriesForCompany])
  }, [categoryResults])
        ...
        <div className="md:grid md:grid-cols-3 md:gap-6">
          <div className="md:col-span-1">
            <div className="px-4 sm:px-0">
              <h3 className="text-lg font-medium leading-6 text-gray-900">{makeName}</h3>
              <p className="mt-1 text-sm text-gray-600">
                Assign Categories to individual models. To make it easy, you can assign all models to the same category using the select box below.
              </p>
              <div className="pt-5">
            ...
            <div className="bg-white shadow sm:rounded-md">
              <ul className="divide-y divide-gray-200 max-h-96 overflow-auto">
                {displayModels.map((model) => (
                  <li key={model.name}>
                    <a onClick={() => handleRowClick(model)} className="cursor-pointer block hover:bg-gray-50">
                      <div className="px-4 py-4 sm:px-6">
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-medium text-cyan-600 truncate">{model.name}</p>
                          <div className="ml-2 flex-shrink-0 flex">
                            {model.category && <p className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                              {model.category.name}
                            </p>}
                          </div>
                        </div>
